...
const PROJECTS = [
  ...
  },
  {
    ...
    category: "データ分析 / ML",
    title: "競馬AI",
    desc: "JRA中央競馬10年分のデータから着順の確率分布を推定し、全券種の期待値を計算してプラス期待値の馬券だけを購入するシステム。",
    story: "「よく当たる予想」を作ることと「回収率がプラスになる」ことは別問題だと分かっていたので、的中率ではなく実際に儲かるかどうかを実測で検証できるシステムを目指した。",
    features: [
      "ウォークフォワード検証により「実際に買えた馬券」の的中率・回収率を実測(2016-2026年 回収率92.9%)",
      "自分の推定確率が市場オッズの示す確率を上回る券種のみ購入対象にする期待値ベース設計",
      "的中率ではなく回収率を最適化する方針を明確化(条件を満たさない日は購入0点)"
    ],
    architecture: [
      "過去N年で学習して翌1年で検証、を1年ずつずらして繰り返すウォークフォワード検証を採用し、「そのとき実際に買えた馬券」ベースの回収率を算出(単純なクロスバリデーションでは未来のデータが混入してしまうため)",
      "モデルの確率精度は高い(302,521頭で予測勝率と実勝率が全十分位で一致)一方、モデルスコアと市場価格(log オッズ)の相関が0.85と高く、市場を知った上でモデルを足しても対数尤度がほぼ改善しないという限界を検証済み",
      "JRAの控除率(20.0〜27.5%)を前提に、自分の推定確率が市場が織り込んだ確率を上回る券種だけを買い、それ以外は見送るという期待値ベースの購入ロジックにした結果、的中率は下がる代わりに回収率を100%近くまで引き上げている"
    ],
    stack: ["Python", "LightGBM", "SQLite", "pandas/parquet"],
    path: "競馬AI/run.py",
    impact: {
      basis: "measured",
      metrics: [
        { label: "回収率(ウォークフォワード検証 2016-2026年)", before: "72.5〜80%", after: "92.9%" },
        { label: "1位指名馬の単勝的中率", before: "約7%", after: "27〜29%" }
      ]
    }
  }
];
